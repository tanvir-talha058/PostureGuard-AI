...
import pytest
from test_session import CRANING, fill
...
def _achievements(store, baseline=None, today=TODAY):
    """compute_achievements() takes an already-computed trend rather than deriving
    one itself — real callers share a single compute_weekly_trend() call across the
    Milestones and trend cards. Tests recompute it here so each case stays a single
    call site."""
    trend = compute_weekly_trend(store, today=today)
    return compute_achievements(store, baseline, trend, today=today)


class TestAchievements:
    def test_five_fixed_milestones_none_earned_on_empty_history(self, store):
        achievements = _achievements(store)
    ...
    def test_first_calibration_earned_once_a_baseline_exists(self, store):
        ...
        achievements = _achievements(store, baseline=baseline)
